...
def get_boundary_coordinates(boundary_data):
    ...
    comma = False               #sets comma False until Y comes
    coX = ""
    coY = ""
    xList = []        
    yList = []    
    
    for r in ((' ', ''), ('z', ''), ('M', 'L'), ('L', ' ')): boundary_data = boundary_data.replace(*r)
    
    for data in boundary_data:
        if(data=="," and comma == False): 
            xList.append(float(coX))
            coX = ""
        if(data==" " and comma == True): 
            yList.append(float(coY))
            coY = ""
        
        if(data == ","): comma = True
        if(data == " "): comma = False
        
        elif(comma==False): coX += data
        elif(comma==True and data != ","): coY += data
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    country_attributes = get_county_attributes(svg_file_name)
    
    with open(csv_file_name, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=',', quoting=csv.QUOTE_MINIMAL)
        for (fips, boundary) in country_attributes:
            center = compute_county_center(get_boundary_coordinates(boundary))
...
process_county_attributes("USA_Counties_2014.svg", "USA_Counties_with_FIPS_and_centers2.csv")                                   
